...
func GrpLSSSShare(S *bn256.G1, AA *Node) ([]*bn256.G1, error) {
	matrix := Convert(AA)
	...
	for i := 0; i < matrixCols-1; i++ {
		v[i+1], _ = rand.Int(rand.Reader, bn256.Order)
	...
	shares := make([]*bn256.G1, matrixRows)
	lambdas, _ := MultiplyMatrix(matrix, v2)
	for i, lambda := range lambdas {
		shares[i] = new(bn256.G1).ScalarMult(S, lambda[0])
	}
	return shares, nil
}
...
func GenerateParityMatrix(M [][]*big.Int, P *big.Int) [][]*big.Int {
	...
	for col := 0; col < n && currentRow < d; col++ {
		...
		invPivot := new(big.Int).ModInverse(pivotVal, P)
		if invPivot == nil {
			panic("Fail to compute ModInverse: Matrix singular or P not prime?")
		}

		for c := 0; c < n; c++ {
			A[currentRow][c].Mul(A[currentRow][c], invPivot).Mod(A[currentRow][c], P)
		}
		for r := 0; r < d; r++ {
