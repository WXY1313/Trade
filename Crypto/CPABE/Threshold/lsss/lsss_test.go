...
func TestLSSS(t *testing.T) {
	...
	P_1 := node.NewNode(false, 3, 2, big.NewInt(int64(2)), "")
	root.Children = []*node.Node{P_A, P_1}
	P_B := node.NewNode(true, 0, 1, big.NewInt(int64(1)), "Attr2")
	P_C := node.NewNode(true, 0, 1, big.NewInt(int64(2)), "Attr3")
	P_D := node.NewNode(true, 0, 1, big.NewInt(int64(3)), "Attr4")
	P_1.Children = []*node.Node{P_B, P_C, P_D}
	...
	P_11 := node.NewNode(false, 2, 2, big.NewInt(int64(2)), "")
	path.Children = []*node.Node{P_A, P_11}
	P_11.Children = []*node.Node{P_B, P_C}
	...
	recoveredSecret, err := ReconG1(path, Q)
	if err != nil {
