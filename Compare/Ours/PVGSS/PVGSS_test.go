...
func TestLSSSPVGSS(t *testing.T) {
	nx := 5        // the number of delegated proxies
	...
	P_buyer := LSSS.NewNode(true, 0, 1, big.NewInt(int64(2)))
	P_pay := LSSS.NewNode(false, 3, 2, big.NewInt(int64(1)))
	root.Children = []*LSSS.Node{P_pay, P_buyer}
	P_seller := LSSS.NewNode(true, 0, 1, big.NewInt(int64(1)))
	P_proxy := LSSS.NewNode(false, nx, tx, big.NewInt(int64(2)))
	P_sub := LSSS.NewNode(true, 0, 1, big.NewInt(int64(3)))
	P_pay.Children = []*LSSS.Node{P_seller, P_proxy, P_sub}
	...
	rows := len(I00)
	cols := len(matrix[0]) // 原始矩阵的列数
	recMatrix := make([][]*big.Int, rows)
	for i := 0; i < rows; i++ {
		recMatrix[i] = make([]*big.Int, rows)

		// 复制已有的数据
		copy(recMatrix[i], matrix[I00[i]][:min(cols, rows)])

		// 补零处理（如果rows > cols）
		for j := cols; j < rows; j++ {
			recMatrix[i][j] = big.NewInt(0)
		}
	}
	fmt.Printf("recMatrix=%v\n", recMatrix)
	invMatrix, _ := LSSS.GaussJordanInverse(recMatrix)
	fmt.Printf("invMatrix=%v x %v\n", len(invMatrix), len(invMatrix[0]))
