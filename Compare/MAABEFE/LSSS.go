...
func Reconstruct(msp *abe.MSP, shareSet map[string]*big.Int, p *big.Int) (*big.Int, error) {
	if len(shareSet) == 0 {
	...
	// SubMatrix初始化MI
	SubMatrix := make(data.Matrix, 0, len(shareSet)-1)
	// indices 存储原始 LSSS 矩阵的行索引，以便与 shares 对应
	indices := make([]int, 0, len(shareSet)-1)
	...
	reconstructedGT := new(bn256.G1).ScalarBaseMult(big.NewInt(0)) //GT群的单位元 1_GT
	...
	for k, wi := range WI {
		...
		// term = (Ai)^{wi}
		term := new(bn256.G1).ScalarMult(shareI, exponent)
