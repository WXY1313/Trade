...
func TestMAABE(t *testing.T) {
	...
	auth1, err := maabe.AuthSetup(pp, "auth1")
	if err != nil {
		t.Fatalf("Failed generation authority %s: %v\n", "auth1", err)
	}
	auth2, err := maabe.AuthSetup(pp, "auth2")
	if err != nil {
		t.Fatalf("Failed generation authority %s: %v\n", "auth2", err)
	}
	auth3, err := maabe.AuthSetup(pp, "auth3")
	if err != nil {
		t.Fatalf("Failed generation authority %s: %v\n", "auth3", err)
	}
	...
	pkSet := []*MAABEPubKey{auth1.PK, auth2.PK, auth3.PK}

	// choose a message
	msg := "Secret"

	// encrypt the message with the decryption policy in msp
	ct, err := maabe.Encrypt(pp, msg, msp, pkSet)
	if err != nil {
	...
	key11, err := maabe.KeyGen(pp, gid, auth1, attrSet1[0])
	//keys1[1]
	if err != nil {
		t.Fatalf("Failed to generate attribute keys: %v\n", err)
	}
	// authority 2 issues keys to user
	key21, err := maabe.KeyGen(pp, gid, auth2, attrSet2[0])
	if err != nil {
		t.Fatalf("Failed to generate attribute keys: %v\n", err)
	}
	// authority 3 issues keys to user
	key31, err := maabe.KeyGen(pp, gid, auth3, attrSet3[0])
	if err != nil {
		t.Fatalf("Failed to generate attribute keys: %v\n", err)
	}

	ks := []*MAABEKey{key11, key21, key31} // ok

	// try to decrypt all messages
	msg1, err := maabe.Decrypt(pp, ct, ks)
	if err != nil {
